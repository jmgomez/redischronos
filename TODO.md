...
- [x] **FEATURE-CACHE-1 — Add a standalone backend-neutral cache module with injectable cancellation/failure policy (TDD).** Create `src/redischronos/cache.nim` as an opt-in module (`import redischronos/cache`) layered only on an injected `KvStore`; keep storage primitives and existing public contracts unchanged and include no consumer/domain imports. RED: freeze `Cache`, `CacheLoader`, `CacheOptions`, `CacheCancellationPolicy`, and `CacheFailurePolicy` through public contract tests covering cold miss→load→fill, hit without loader, TTL, explicit invalidation/version bump, loader failure not cached, backend timeout/error policy, concurrent same-key miss coalescing, one cancelled waiter with surviving waiters, last-waiter cancellation policy, and close during an active fill. GREEN: implement `getOrLoad`, cache read/write/invalidate and generic versioned-key helpers with one owned in-flight load per key; a cancelled caller must stop waiting promptly without cancelling a load still needed by others, policy decides whether an orphaned shared load continues, and `CancelledError` is never converted into a miss. ACCEPT: the unchanged cache contract passes with injected in-memory and real-Redis `KvStore` instances under refc/ORC and the minimum supported Nim version; switching `mem://` to `redis://` requires configuration only; close is cancellation-safe/join-idempotent and leaves no loader, waiter, key-state, or backend task alive.
- [x] **AUDIT2-CACHE-C1 — Fence active fills against invalidation and close (TDD).** RED: block a cold-miss loader, invalidate or bump the key version, then release it; separately start close while a cancellation-resistant loader is active. Assert neither old fill can publish a value or resolve a waiter successfully after the fence. GREEN: assign per-key/cache generations (or equivalent ownership tokens) and validate them immediately before backend write and waiter success; invalidation/close advances the fence. ACCEPT: no stale value can be resurrected after invalidate/version bump and no cache write or successful fill occurs after close begins, under refc/ORC and both backends.
- [x] **AUDIT2-CACHE-C2 — Make cache close own every in-flight read, fill, and waiter (TDD).** RED: block the backend cache read before loader creation, attach multiple waiters, race concurrent/cancelled close calls, and include a loader that delays cancellation. GREEN: register operations before their first await, publish one shielded terminal cleanup future, cancel/wake all waiters deterministically, and retain task ownership until each task actually exits. ACCEPT: close is join-idempotent, callers receive the documented terminal error, and no backend read, loader, waiter, in-flight entry, or cleanup task survives completed close.
- [x] **AUDIT2-CACHE-C3 — Handle reentrant loads and cancelled-orphan replacement safely (TDD).** RED: have a loader call getOrLoad for its own key and prove a typed deterministic failure rather than a nil-future SIGSEGV; cancel the last waiter under each cancellation policy and immediately add a replacement waiter. GREEN: publish only fully initialized in-flight entries, detect same-task recursion, and atomically retire/replace cancelled orphan entries. ACCEPT: no nil task reaches allFutures, no replacement inherits a dying cancelled load, one physical load is retained for valid concurrent waiters, and retries work under refc/ORC.
- [x] **AUDIT2-PUBSUB-C1 — Make awaited close safe from inside owned callbacks (TDD).** RED: from a yielding message handler and state observer, await bus.close() under a deadline for memory and Redis implementations. GREEN: make callback-aware shutdown avoid joining the currently executing worker while preserving one terminal cleanup future and deterministic retirement. ACCEPT: callback-initiated awaited close completes without self-deadlock, no callback begins afterward, concurrent external close callers converge, and no worker/observer task survives shutdown.
- [x] **AUDIT2-CACHE-W1 — Apply CacheFailurePolicy consistently across every backend operation (TDD).** RED: inject read, write, delete/invalidate, and version-read/version-increment failures including BackendTimeoutError under fail-open and strict policies; include standalone -d:redisIntegration with no Redis URL. GREEN: route every cache-owned backend operation, including bumpVersion, through the declared policy and make integration mode fail fast when Redis is unavailable. ACCEPT: strict mode propagates typed errors, fail-open behavior is explicitly documented/tested per operation, CancelledError is never swallowed, and Redis-labelled cache tests cannot silently run against memory.
- [x] **AUDIT3-W1 — Bound callback-close ancestry tracking (TDD).** RED: call close repeatedly after terminal shutdown and from concurrent message/state callbacks, then assert ancestry/caller tracking returns to a fixed baseline. GREEN: retain only active caller relationships and remove them on completion, or replace closeCallers with a non-growing execution-context mechanism. ACCEPT: ten thousand idempotent close calls do not grow retained futures in either Pub/Sub backend and callback-aware shutdown behavior remains correct.
- [x] **AUDIT3-W2 — Bound ownership of timed-out DNS work (TDD/stress).** RED: inject many slow resolutions that exceed their deadlines and measure outstanding resolver workers/queued jobs after callers fail. GREEN: use an owned bounded resolver/task-pool mechanism with late-result disposal instead of abandoning std/threadpool FlowVars. ACCEPT: repeated DNS timeouts keep worker and queue counts bounded, the event loop remains responsive, typed errors are preserved, and builds no longer rely on the deprecated threadpool module.
- [x] **AUDIT3-W3 — Synchronize project rules and design layout with the approved cache module.** Update CLAUDE.md so the library owns the generic injected-KvStore cache mechanism while consumers own domain keys/invalidation policy; update docs/design.md package layout for cache.nim, redisresolve.nim, and current contract/fault tests. ACCEPT: architecture instructions no longer prohibit or misassign the implemented module and the authoritative layout matches the repository.
- [x] **AUDIT4-CACHE-C1 — Make stale-fill cleanup replacement-safe and terminal failures observable (TDD).** RED: park an old fill inside a cancellation-resistant `store.set`, cancel its last waiter under `ccpCancelOrphaned`, publish an immediate replacement for the same key, then release the old write and assert the replacement survives; repeat with cache close, a newer same-key owner, and an injected compensating-delete `BackendConnectionError`, asserting close cannot report success while stale data remains. GREEN: serialize prior physical-write retirement before a new logical-key owner can publish, or use an atomic ownership-token/conditional cleanup mechanism; never blindly delete a key that may belong to a newer fill, and route cleanup failures through explicit strict/fail-open semantics without losing typed errors. ACCEPT: same-cache and shared-store replacements survive every old-fill completion order, successful close leaves no stale write, failed cleanup is observable as documented, and focused plus full memory/live-Redis refc/ORC suites pass.
- [x] **AUDIT4-PUBSUB-C1 — Make terminal state-observer close re-entry deadlock-free (TDD).** RED: use the same yielding `StateHandler` on memory and Redis that awaits `bus.close()` for every notification, including `csClosed`; start callback-initiated and concurrent external closes and prove all calls converge under a deterministic deadline with exactly one ordered terminal notification and no surviving observer task. GREEN: keep callback-aware close handoff valid for every observer invocation created while shutdown drains, or otherwise prevent terminal notification dispatch from re-entering the in-progress close cycle without weakening state semantics. ACCEPT: unconditional observer close, nested helpers, concurrent close callers, and repeated terminal close all finish under refc/ORC on both backends with zero retained close callers/workers.
- [x] **AUDIT4-TEST-W1 — Measure resolver ownership instead of asserting a constant (TDD/stress).** RED: make the resolver stress test fail when an extra worker is created or retained; remove the hard-coded worker count from `resolverStateForTest` and observe actual worker creation/alive state at the production ownership boundary. GREEN: add minimal thread-safe test instrumentation without changing the documented process-lifetime worker design. ACCEPT: repeated timeout/capacity waves prove the real per-event-loop-thread worker count stays at its documented steady state, request/response queues stay bounded, late results drain, and refc/ORC remain responsive.
